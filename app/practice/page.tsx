...
export default function PracticePage() {
    ...
    const generateParagraph = async () => {
        ...
        try {
            ...
            const sentenceStates: SentenceState[] = data.sentences.map((s: string) => ({
                original: s,
                userTranslation: '',
                evaluation: null,
                isCompleted: false,
            ...
            setError(err instanceof Error ? err.message : 'Có lỗi xảy ra khi tạo đoạn văn');
        } finally {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Có lỗi xảy ra khi chấm điểm');
        } finally {
    ...
    const isAllCompleted = sentences.length > 0 && completedCount === sentences.length;
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    {!isGenerating && sentences.length > 0 && (
                        <>
                            ...
                            <div className="space-y-4 mb-6">
                                {sentences.map((sentence, index) => (
                                    <div
                                        key={index}
                                        className={`card p-5 transition-all duration-300 animate-slide-in ${index === currentIndex
                      ...
                      `}>
                                            ...
                                            {sentence.evaluation && (
                                                <span className={`
                          badge ${sentence.evaluation.isCorrect ? 'badge-success' : 'badge-warning'}
                        `}>
                                        ...
                                        {sentence.isCompleted && sentence.evaluation && (
                                            <div className="mt-4 space-y-3 animate-fade-in">
                                                ...
                                                <div className="p-3 rounded-lg bg-border-light">
                                                    <p className="text-sm text-foreground-muted mb-1">Bản dịch của bạn:</p>
                                                    <p className="text-foreground">{sentence.userTranslation}</p>
                                                </div>

                                                {/* Feedback */}
                                                <div className={`p-3 rounded-lg ${sentence.evaluation.isCorrect ? 'bg-success/10' : 'bg-warning/10'
                                                    }`}>
                                                ...
                                                {/* Errors */}
                                                {sentence.evaluation.errors && sentence.evaluation.errors.length > 0 && (
                                                    <div className="p-3 rounded-lg bg-error/5">
                                                        <p className="text-sm font-medium mb-2 text-error">Các lỗi cần sửa:</p>
                                                        <ul className="space-y-2">
                                                            {sentence.evaluation.errors.map((err, errIdx) => (
                                                                <li key={errIdx} className="text-sm">
                                                                    <span className="text-error line-through">{err.error}</span>
                                                                    <ArrowRight className="w-3 h-3 inline mx-2 text-foreground-muted" />
                                                                    <span className="text-success">{err.correction}</span>
                                                                    {err.explanation && (
                                                ...
                                                <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
                                                    <p className="text-sm font-medium mb-1 text-primary">Bản dịch mẫu:</p>
                                                ...
                                                {/* Next Button */}
                                                {index === currentIndex && index < sentences.length - 1 && (
                                                    <button
                            ...
                            {isAllCompleted && (
                                <div className="card p-8 text-center animate-fade-in bg-gradient-to-br from-primary/5 to-secondary/5">
                                    ...
                                    <p className="text-foreground-muted mb-4">
                                        Bạn đã hoàn thành {sentences.length} câu với điểm trung bình {averageScore}/10
                                    </p>
                                    <button
                                        onClick={generateParagraph}
                                        className="btn btn-primary"
